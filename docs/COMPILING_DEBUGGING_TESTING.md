...
The intended audience are developers, who want to leverage newly added tricks to Prolo via `CMake`. The document will lower the entry point for these developers.
...
Prolo can be compiled via the main `Makefile`, using one of several targets listed there.
...
It is possible to extract these `CMake` calls and modify them for your specific needs. For example, a minimal external cmake command to compile Prolo, executed from within a newly created build directory could look like:

`cmake -S "$DIR_SRC" -DCMAKE_BUILD_TYPE=Release && make`

where the variable `DIR_SRC` is expected to store the path to the Prolo source code.
...
The additional artifact of the above call is the `prolo.cbp` Code::Blocks project file in the build directory.
...
After a successful build, open the `prolo.cbp` with CB. From the CB's menu bar select the target, that you want debug. Assuming these are unit tests:
...
https://github.com/prolo-project/prolo/pull/7160

### Faster core_tests with caching
https://github.com/prolo-project/prolo/pull/5821

### Precompiled headers
https://github.com/prolo-project/prolo/pull/7216

### Unity builds
https://github.com/prolo-project/prolo/pull/7217
