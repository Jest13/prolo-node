...
# Copyright (c) 2022-2024 The Prolo Project
...
# target, so we don't spend time building Prolo binaries.
...
GIT_ARCHIVE="${DIST_ARCHIVE_BASE}/prolo-source-${VERSION}.tar.gz"
...
if [ ! -e "$GIT_ARCHIVE" ]; then
    ...
    | sort \
    | tar --create --transform "s,^,prolo-source-${VERSION}/," --mode='u+rw,go+r-w,a+X' --files-from=- \
    | gzip -9n > ${GIT_ARCHIVE}
...
(
    ...
    # Setup the directory where our Prolo build for HOST will be
