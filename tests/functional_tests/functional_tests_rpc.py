...
# a main offline prolod, does most of the tests
# a restricted RPC prolod setup with RPC payment
# two local online prolods connected to each other
N_PROLODS = 5

# 4 wallets connected to the main offline prolod
# 1 wallet connected to the first local online prolod
# 1 offline wallet
N_WALLETS = 7
...
prolod_base = [builddir + "/bin/prolod", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--no-igd", "--p2p-bind-port", "prolod_p2p_port", "--rpc-bind-port", "prolod_rpc_port", "--zmq-rpc-bind-port", "prolod_zmq_port", "--zmq-pub", "prolod_zmq_pub", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--data-dir", "prolod_data_dir", "--log-level", "1", "--rpc-max-connections-per-private-ip", "100", "--rpc-max-connections", "100"]
prolod_extra = [
  ["--offline"],
...
wallet_base = [builddir + "/bin/prolo-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--log-level", "1", "--allow-mismatched-daemon-version"]
wallet_extra = [
...
for i in range(N_PROLODS):
  command_lines.append([str(18180+i) if x == "prolod_rpc_port" else str(18280+i) if x == "prolod_p2p_port" else str(18380+i) if x == "prolod_zmq_port" else "tcp://127.0.0.1:" + str(18480+i) if x == "prolod_zmq_pub" else builddir + "/functional-tests-directory/prolod" + str(i) if x == "prolod_data_dir" else x for x in prolod_base])
  if i < len(prolod_extra):
    command_lines[-1] += prolod_extra[i]
  outputs.append(open(FUNCTIONAL_TESTS_DIRECTORY + '/prolod' + str(i) + '.log', 'a+'))
