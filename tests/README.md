...
Core tests take longer than any other Prolo tests, due to the high amount of computational work involved in validating core components.
...
To run only Prolo's core tests (after building):
...
To run only Prolo's crypto tests (after building):
...
```bash
prolod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `prolo-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Prolo's hash tests (after building):
...
To run only Prolo's performance tests (after building):
...
To run only Prolo's unit tests (after building):
